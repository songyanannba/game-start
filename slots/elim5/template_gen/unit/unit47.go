...
type Slot47 struct {
	...
	testSlot47 *unit5.Unit
}

func GetUnit47(t *gen_com.GenTemplate) *Slot47 {
	return &Slot47{
		testSlot47: unit5.NewUnit(),
		gTem:       t,
	}
}

func (s *Slot47) RunTem() ([]*component.Spin, error) {
	sp := &component.Spin{
...
// Calculate 累加数据
func (s *Slot47) Calculate(spins []*component.Spin) {
	s.testSlot47.Calculate(spins)
}

// GetLogicResult slot8 计算数据
func (s *Slot47) GetLogicResult() *gen_com.LogicResult {
	res := s.testSlot47.Result
	str := s.testSlot47.GetDetail()
	fmt.Println(str)
...
func (s *Slot47) GetStatus() (totalInterval float64, str string, ok bool) {
	r := s.GetLogicResult()
...
// Adjust slot8 调整数据
func (s *Slot47) Adjust(genc *gen_com.LogicResult) (str string, ok bool) {
