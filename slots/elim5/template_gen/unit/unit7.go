...
import (
	...
	"elim5/service/test/handler/repeat/unit7"
	"elim5/utils/helper"
	"fmt"
)
...
type Slot7 struct {
	...
	testSlot15 *unit7.Unit
}

func GetUnit7(t *gen_com.GenTemplate) *Slot7 {
	return &Slot7{
		testSlot15: unit7.NewUnit(),
		gTem:       t,
	}
}

func (s *Slot7) RunTem() ([]*component.Spin, error) {
	sp := &component.Spin{
...
// Calculate 累加数据
func (s *Slot7) Calculate(spins []*component.Spin) {
	s.testSlot15.Calculate(spins)
}

// GetLogicResult slot8 计算数据
func (s *Slot7) GetLogicResult() *gen_com.LogicResult {
	res := s.testSlot15.Result
	str := s.testSlot15.GetDetail()
	fmt.Println(str)
...
func (s *Slot7) GetStatus() (totalInterval float64, str string, ok bool) {
	r := s.GetLogicResult()
...
// Adjust slot8 调整数据
func (s *Slot7) Adjust(genc *gen_com.LogicResult) (str string, ok bool) {
	//global.GVA_LOG.Info(fmt.Sprintf("%v", genc))
	condKeys := []string{gen_com.RemoveRateCond, gen_com.WinRateCond, gen_com.GainRatioCond}
	var adjust string
