...
import (
	...
	"elim5/service/test/handler/repeat/unit8"
	"elim5/utils/helper"
	"fmt"
)
...
func GetUnit8(t *gen_com.GenTemplate) *Slot8 {
	return &Slot8{
		testSlot: unit8.NewUnit(),
		gTem:     t,
	}
}

func (s *Slot8) RunTem() ([]*component.Spin, error) {
	sp := &component.Spin{
...
// Calculate 累加数据
func (s *Slot8) Calculate(spins []*component.Spin) {
	s.testSlot.Calculate(spins)
}

// GetLogicResult slot8 计算数据
func (s *Slot8) GetLogicResult() *gen_com.LogicResult {
...
func (s *Slot8) GetStatus() (totalInterval float64, str string, ok bool) {
	r := s.GetLogicResult()
...
// Adjust slot8 调整数据
func (s *Slot8) Adjust(genc *gen_com.LogicResult) (str string, ok bool) {
