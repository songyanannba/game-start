...
import (
	...
	"elim5/service/test/handler/repeat/unit54"
	"elim5/utils/helper"
	"fmt"
)
...
func GetUnit54(t *gen_com.GenTemplate) *Slot54 {
	return &Slot54{
		test: unit54.NewUnit(),
		gTem: t,
	}
...
func (s *Slot54) RunTem() ([]*component.Spin, error) {
	sp := &component.Spin{
...
// Calculate 累加数据
func (s *Slot54) Calculate(spins []*component.Spin) {
	s.test.Calculate(spins)
}

// GetLogicResult slot8 计算数据
func (s *Slot54) GetLogicResult() *gen_com.LogicResult {
	res := s.test.Result
	str := s.test.GetDetail() + s.test.Result.SummaryData.String()
	fmt.Println(str)
...
func (s *Slot54) GetStatus() (totalInterval float64, str string, ok bool) {
	r := s.GetLogicResult()
...
// Adjust slot8 调整数据
func (s *Slot54) Adjust(genc *gen_com.LogicResult) (str string, ok bool) {
