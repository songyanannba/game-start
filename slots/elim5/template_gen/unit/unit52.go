...
import (
	...
	"elim5/service/test/handler/repeat/unit52"
	"elim5/utils/helper"
	"fmt"
)
...
func GetUnit52(t *gen_com.GenTemplate) *Slot52 {
	return &Slot52{
		test: unit52.NewUnit(),
		gTem: t,
	}
...
func (s *Slot52) RunTem() ([]*component.Spin, error) {
	sp := &component.Spin{
...
// Calculate 累加数据
func (s *Slot52) Calculate(spins []*component.Spin) {
	s.test.Calculate(spins)
}

// GetLogicResult slot8 计算数据
func (s *Slot52) GetLogicResult() *gen_com.LogicResult {
	res := s.test.Result
...
func (s *Slot52) GetStatus() (totalInterval float64, str string, ok bool) {
	r := s.GetLogicResult()
...
// Adjust slot8 调整数据
func (s *Slot52) Adjust(genc *gen_com.LogicResult) (str string, ok bool) {
