...
import (
	...
	"elim5/service/test/handler/repeat/unit33"
	"elim5/utils/helper"
	"fmt"
)
...
type Slot33 struct {
	...
	testSlot *unit33.Unit
}

func GetUnit33(t *gen_com.GenTemplate) *Slot33 {
	return &Slot33{
		testSlot: unit33.NewUnit(),
		gTem:     t,
	}
}

func (s *Slot33) RunTem() ([]*component.Spin, error) {
	sp := &component.Spin{
...
// Calculate 累加数据
func (s *Slot33) Calculate(spins []*component.Spin) {
	s.testSlot.Calculate(spins)
}

// GetLogicResult slot33 计算数据
func (s *Slot33) GetLogicResult() *gen_com.LogicResult {
	...
	case enum.SpinAckType1BranchBuyFree:
		return &gen_com.LogicResult{
			ScatterRatio: float64(res.FreeSca) / float64(res.FreeCount),
			GainRatio:    (float64(res.FreeWin) + float64(res.NormalWin)) / (float64(res.N2) * s.gTem.Config.BuyFee),
			WinRatio:     float64(res.FreeWinCount) / float64(res.FreeCount),
			RemoveRate:   float64(res.N79+res.N81+res.N83) / float64(res.N97),
		}
	default:
...
func (s *Slot33) GetStatus() (totalInterval float64, str string, ok bool) {
	r := s.GetLogicResult()
...
// Adjust slot33 调整数据
func (s *Slot33) Adjust(genc *gen_com.LogicResult) (str string, ok bool) {
