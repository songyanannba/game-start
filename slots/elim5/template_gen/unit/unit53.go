...
import (
	...
	"elim5/service/test/handler/repeat/unit53"
	"elim5/utils/helper"
	"fmt"
)
...
func GetUnit53(t *gen_com.GenTemplate) *Slot53 {
	return &Slot53{
		test: unit53.NewUnit(),
		gTem: t,
	}
...
func (s *Slot53) RunTem() ([]*component.Spin, error) {
	sp := &component.Spin{
...
// Calculate 累加数据
func (s *Slot53) Calculate(spins []*component.Spin) {
	s.test.Calculate(spins)
}

// GetLogicResult slot8 计算数据
func (s *Slot53) GetLogicResult() *gen_com.LogicResult {
	res := s.test.Result
	str := s.test.GetDetail()
	fmt.Println(str)
...
func (s *Slot53) GetStatus() (totalInterval float64, str string, ok bool) {
	r := s.GetLogicResult()
...
// Adjust slot8 调整数据
func (s *Slot53) Adjust(genc *gen_com.LogicResult) (str string, ok bool) {
