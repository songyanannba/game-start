...
// FormatCommandStr 清空前后空格，多个空格替换为单个空格
...
func BuildOrderNo() string {
	microsecondOrderNoKey := "{orderNo}:"
	orderNo := "TX" + time.Now().Format("060102150405") + RandomString(8)
	//orderNo = BuildMicrosecondOrderNo()
	ok, err := global.GVA_REDIS.SetNX(context.Background(), microsecondOrderNoKey+orderNo, 1, 5*time.Second).Result()
	if err == nil && !ok {
