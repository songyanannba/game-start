...
func (t *Table) FindLine(verify *Verify) []*base.Tag {
	...
	for _, tag := range nearbys {
		if tag.Name == "" || tag.Name == enum.ScatterName || verify.GetVerify(tag.X, tag.Y) {
			continue
		}
...
// FindAllLine 获取所有的赢钱划线 （上下左右>=5个相同标签）
...
func (t *Table) FindMayLine(verify *Verify) []*base.Tag {
	...
	for _, tag := range nearbys {
		if tag.Name == enum.ScatterName || verify.GetVerify(tag.X, tag.Y) {
			continue
		}
...
func (t *Table) FindQuantityTags(num int) [][]*base.Tag {
	...
	for tagName, tags := range tagsMap {
		if len(tags) >= num && tagName != "" && tagName != enum.ScatterName && tagName != enum.MultiplierName {
