...
func (t *Table) MatchWinMul(tags []*base.Tag) decimal.Decimal {
	...
	if payTable, ok := payTableLists[len(tags)]; ok {
		...
		// 向下兼容 匹配最近一个
		for i := len(tags); i >= enum.GetLine; i-- {
...
func (t *Table) WinMatch6(tagList [][]*base.Tag) ([]*base.Eliminate, float64) {
	var eliminates []*base.Eliminate
	sumMul := float64(0)
	for _, tags := range tagList {
		//mul := t.WinMatch(tags)
		//获取匹配画线的赔率
		mul := t.MatchWinMul(tags) //WinMatch6

		//wild倍率
		for _, tag := range tags {
			if tag.IsWild {
				for _, singleTags := range t.SingleTags {
					lastTag := singleTags.PosOder[len(singleTags.PosOder)-1]
					if lastTag.X == tag.X && tag.Y == lastTag.Y {
						mul = mul.Mul(decimal.NewFromFloat(float64(singleTags.SubId)))
					}
				}
			}
		}
