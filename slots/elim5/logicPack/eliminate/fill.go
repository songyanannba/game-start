...
func (t *Table) Fill(isFee bool, stop bool) (rTags []*base.Tag) {
	if stop {
		//return t.FillAll()
	}
	//lineList 需要消除的集合 （标签大于等于5个的集合）
	//lineList 和赢钱的组合去匹配 ,返回集合中 每种要消除划线的集合 和对应的倍率
	_, mul := t.WinMatchList(t.FindAllLine(), isFee)
...
func (t *Table) MatSuitablePTables() []*base.PayTable {
	var pTables []*base.PayTable
	switch t.SlotId {
	case enum.SlotId5:
	...
	if len(pTables) == 0 {
		pTables = lo.Filter(t.PayTableList, func(item *base.PayTable, index int) bool {
			return item.Multiple <= t.Target.MaxMul
		})
		if len(pTables) == 0 {
			global.GVA_LOG.Error(fmt.Sprintf("InitFill:没有符合条件的初始填充, MaxMul:%g InitNum:%d", t.Target.MaxMul, t.Target.InitNum))
		}
	}
	return pTables
}
