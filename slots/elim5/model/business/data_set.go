...
// TableName DataSet 表名
func (DataSet) TableName() string {
	return "b_data_set"
}

func (d *DataSet) Conversion() {
...
func (d *DataSet) GetPreviousUsers() DataSet {
	var dataSet DataSet
	global.NOLOG_DB.Model(&DataSet{}).
		Where("`type` =? and set_id = ? ",
			d.Type, d.SetId).
		Where("date < ?", d.Date).
		Order("date desc").
		First(&dataSet)
