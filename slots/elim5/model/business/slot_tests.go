...
type SlotTests struct {
	...
	Status   uint8  `json:"status" form:"status" gorm:"column:status;default:1;comment:状态;size:8;index"`
	...
	Rtp      int    `json:"rtp" form:"rtp" gorm:"column:rtp;default:0;comment:rtp;size:32;"`
	Config   []byte `json:"config" form:"config" gorm:"column:config;comment:配置;type:blob"` // 配置
}
