...
func (m *Machine) PlayGame(count, rank int, reSpin int) {
	...
	//获取当前 要消除的划线集合
	lineList := m.Table.FindAllLineV2() //6
	//
	...
	AlterFlow.RemoveList, AlterFlow.SumMul = m.Table.WinMatchList(m.Table.EliminateV2(m.Table.FindAllLineV2()), false)
	...
	m.WildBeat()
	AlterFlow.AfterDrop = m.Table.Drop6()
