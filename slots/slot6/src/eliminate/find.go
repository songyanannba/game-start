...
func (t *Table) FindAllLineV2() [][]*base.Tag {
	tagList := make([][]*base.Tag, 0)
	verify := NewVerify()
	//获取整体的标签数量作为分析的依据
	//tagListKeyNum := t.GetTagListKeyNum()
	for _, tags := range t.TagList {
		for _, tag := range tags {
			//if tag.Name != "" && !verify.GetVerify(tag.X, tag.Y) && !tag.IsWild && tagListKeyNum[tag.Name] >= 5 {
	...
	for _, tag := range tags {
		if tag.Name != "" && !verify.GetVerify(tag.X, tag.Y) {
			...
			for i := 5; i <= len(getTags); i++ {
				mul, _ := t.WinMatchV2(getTags).Float64()
