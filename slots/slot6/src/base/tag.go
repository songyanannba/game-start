...
func (t *Tag) Dump(typ ...int) string {
	...
	if len(typ) > 0 {
		...
		name = strconv.Itoa(t.Id) + helper2.If(t.Multiple > 1, "@"+strconv.Itoa(int(t.Multiple)), "")
	}
	strconv.ParseInt(name, 10, 64)
	attr := ""
	attr += helper2.If(t.IsLine, "1", "0")
	attr += helper2.If(t.IsPayTable, "1", "0")
	attr += helper2.If(t.IsWild, "1", "0")
	attr += helper2.If(t.IsSingle, "1", "0")
	attr += helper2.If(t.IsJackpot, "1", "0")
	attr += helper2.If(t.ISLock, "1", "0")
	if attr != "" {
		name += "|" + helper2.BinToDec("1"+attr)
	}
	return name
}
...
func (t *Tag) parseAttr(s string) {
	...
	bin := helper2.DecToBin(attr)
	attrs := helper2.SplitInt[int](bin, "")
	arr := make([]int, 5)
	copy(arr, attrs)
	t.Id = helper2.StrToInt(id)
	t.IsLine = arr[0] == 1
...
func NewTag(id int, v string, multiple float64, single uint8, include ...string) *Tag {
	...
	return &Tag{
		...
		IsWild:   isWild,
		Multiple: multiple,
		IsSingle: helper2.If(single == 1, true, false),
	}
}
...
func GetTagsName(tags []*Tag, exclude ...string) string {
	...
	for _, tag := range tags {
		if helper2.InArr(tag.Name, exclude) {
			continue
		}
		return tag.Name
	}
	...
	for _, tag := range tagList[0] {
		if len(selectTags) != 0 {
			if tag.InTags(selectTags) {
				sameTags = append(sameTags, newSameTag(tag, 0, ""))
			}
		} else {
			sameTags = append(sameTags, newSameTag(tag, 0, ""))
		}
...
func GetSpecialTags(tagList [][]*Tag, name string) []*Tag {
...
func (t Tag) GetRecordStr(id int, typ string) string {
	...
	return fmt.Sprintf("%s,%d,%d,%d,%d,%f",
		typ, id, t.X, t.Y, helper2.If(t.IsWild, 1, 0), t.Multiple,
	)
}
